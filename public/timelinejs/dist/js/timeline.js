...
( function( $ ) {
	Timeline = {
		init: function (options, elem) {
			...
			//额外新增dom元素
			$(".timeline-dots").width($(".timeline-dots-wrap.bottom").width()-150);//时间刻度条宽度
			$(".timeline-dots").css("margin-left",($(".timeline-dots-wrap.bottom").width()-$(".timeline-dots").width())/2.0+20+"px");
			
			$('.timeline-dots-wrap.bottom').append('<a class="play"><span></span></a>');
			$('.timeline-dots-wrap.bottom').append('<a class="prev"><span></span></a>');
			$('.timeline-dots-wrap.bottom').append('<a class="next"><span></span></a>');
			$('.timeline-dots-wrap.bottom').append('<a class="pause"><span></span></a>');

			//动态计算时间刻度li宽度width值大小
			...
			$(".timeline-dots-wrap.bottom .pause span").on('click', function (e) {
               self.autoplay_clear();
			   $(".timeline-dots-wrap.bottom .play span").show();
               $(".timeline-dots-wrap.bottom .pause span").hide();
			});
			//播放上一步按钮点击事件
			$(".timeline-dots-wrap.bottom .prev span").on('click', function (e) {
			   //判断是不是第一个节点
			   if(self.get_current() > 0){
				 self.options.startItem = self.get_prev();
			   }
			   else{
                 self.options.startItem = self.get_count() - 1;
			   }
			   //self.update_ui();
			   self.autoplay_clear();
			   self.change_slide('click', self.options.startItem);
			   //self.change_slide();
			});
			//播放下一步按钮点击事件
			$(".timeline-dots-wrap.bottom .next span").on('click', function (e) {
			   //判断是不是最后一个节点
			   if(self.get_count() - 1 > self.get_current()){
				 self.options.startItem = self.get_next();
			   }
			   else{
                 self.options.startItem = 0;
			   }
			   //self.update_ui();
			   self.autoplay_clear();
			   self.change_slide('click', self.options.startItem);
