(function() {
  ...
  EventDispatcher.prototype.addEventListener = EventDispatcher.prototype.on = function(
    type,
    callback,
    context,
    priority
  ...
  cli.isFile = cli.isDirectory = function(name) {
    return false;
  };
  ...
  function WriteFilesProxy(gui) {
    ...
    internal.writeFiles = function(files, opts, done) {
      ...
      if (!utils.isArray(files) || files.length === 0) {
        ...
        filename =
          utils.getCommonFileBase(utils.pluck(files, "filename")) || "output";
  ...
  internal.initProjLibrary = function(opts, done) {
    ...
    var libs = internal.findProjLibs(
      [opts.from || "", opts.match || "", opts.crs || ""].join(" ")
    );
    // skip loaded libs
  ...
  */
  ...
  function MultiScaleArcCollection(unfilteredArcs) {
    ...
    if (size > 5e5) {
      if (!!unfilteredArcs.getVertexData().zz) {
        ...
        filteredArcs = internal.simplifyArcsFast(
          unfilteredArcs,
          filteredSegLen
        );
      }
  ...
  function projectDisplayLayer(lyr, displayCRS) {
    ...
    lyr2 = getMapLayer(lyr.source.layer, lyr.source.dataset, {
      crs: displayCRS
    });
  ...
  function getMapLayer(layer, dataset, opts) {
    ...
    if (dataset.arcs && !displayArcs) {
      ...
      if (needReprojectionForDisplay(sourceCRS, displayCRS)) {
        displayArcs = projectArcsForDisplay(
          dataset.arcs,
          sourceCRS,
          displayCRS
        );
  ...
  var MapStyle = (function() {
    ...
    function getOverlayStyle(lyr, o) {
      ...
      if (internal.layerHasCanvasDisplayStyle(lyr)) {
        if (type == "point") {
          overlayStyle = internal.wrapOverlayStyle(
            internal.getCanvasDisplayStyle(lyr),
            overlayStyle
          );
        }
  ...
  function renderFurniture(lyr, ext) {
    ...
    obj.children = internal.svg.importFurniture(
  ...
  function LayerStack(gui, container, ext, mouse) {
    ...
    function layerUsesSVG(layer) {
      return (
        internal.layerHasLabels(layer) || internal.layerHasSvgSymbols(layer)
      );
    }
  ...
  function getShapeHitTest(displayLayer, ext) {
    ...
    function sortByDistance(x, y, cands, arcs) {
      for (var i = 0; i < cands.length; i++) {
        cands[i].dist = geom.getPointToShapeDistance(
          x,
          y,
          cands[i].shape,
          arcs
  ...
  function HitControl2(gui, ext, mouse) {
    ...
    function updateHitData(newData) {
      ...
      storedData = newData;
      gui.container
        .findChild(".map-layers")
        .classed("symbol-hit", newData.ids.length > 0);
      if (active) {
        triggerHitEvent("change");
      }
    }
    ...
    function eventIsEnabled(type) {
      ...
      if (
        ...
        !gui.interaction.modeUsesDrag(mode)
      ) {
        return false;
      }
      return true;
  ...
  function Popup(gui, onNext, onPrev) {
    ...
    self.show = function(id, ids, table, pinned, editable) {
      var rec = table
        ? editable
          ? table.getRecordAt(id)
          : table.getReadOnlyRecordAt(id)
        : {};
    ...
    function showNav(id, ids, pinned) {
      ...
      prevLink.css(
        "display",
        pinned && ids.length > 2 ? "inline-block" : "none"
      );
      tab.show();
    }
  ...
  */
  ...
  function MshpMap(gui) {
    ...
    this.setDisplayCRS = function(crs) {
      ...
      if (_intersectionLyr)
        _intersectionLyr = projectDisplayLayer(_intersectionLyr, newCRS);
      if (_overlayLyr) {
    ...
    function drawLayers(onlyNav) {
      ...
      if (!(_ext.width() > 0 && _ext.height() > 0)) {
        ...
        console.error(
          "[drawLayers()] Collapsed map container, unable to draw."
        );
        return;
      }
      if (!onlyNav) {
  ...
  function InteractionMode(gui) {
    ...
    if (gui.options.inspectorControl) {
      ...
      menu = El("div")
        .addClass("nav-sub-menu")
        .appendTo(btn2.node().parentNode);
    ...
    function renderMenu(modes) {
      ...
      El("div")
        ...
        .text("interactive editing:")
        .appendTo(menu);
      modes.forEach(function(mode) {
        var link = El("div")
          .addClass("nav-menu-item nav-menu-link")
          .attr("data-name", mode)
          .text(labels[mode])
          .appendTo(menu);
  ...
  function GuiInstance(container, opts) {
    ...
    gui.showProgressMessage = function(msg) {
      ...
      El("<div>")
        .text(msg)
        .appendTo(gui.progressMessage.empty().show());
    };
  ...
  var ImportDataBaseControl = function(gui) {
    var model = gui.model;
    var unsupportedMsg = "该浏览器不支持数据入库";
    var importdatabaseBtn = gui.container.findChild(".import-database-btn");
    if (!GUI.exportIsSupported()) {
      gui.alert(unsupportedMsg);
    }
    importdatabaseBtn.on("click", function() {
      // var opts = GUI.parseFreeformOptions(menu.findChild('.advanced-options').node().value, 'o');
      ...
      // if (dataset) {
      //   gui.showProgressMessage('数据入库操作正在导入中,请稍候……');
      //   setTimeout(() => {
      //     gui.clearProgressMessage();
      //   }, 5000);
      //   var opts = { format: "geojson" };
      //   // var geojson = internal.exportDatasetAsGeoJSON(dataset, opts, 'buffer');
      //   var geojson = internal.exportDatasetAsGeoJSON(dataset, opts);
      //   // console.log(geojson);
...
*/
      ...
      setTimeout(function() {
        ...
        try {
          ...
          if (fileType == "shp") {
            ...
            input.shx =
              cachedFiles[fileName.replace(/shp$/i, "shx").toLowerCase()] ||
              null;
          }
  ...
  var ExportControl = function(gui) {
    ...
    function exportMenuSelection(done) {
      var opts, files;
      try {
        opts = GUI.parseFreeformOptions(
          menu.findChild(".advanced-options").node().value,
          "o"
        );
        if (!opts.format) opts.format = getSelectedFormat();
    ...
    function initLayerMenu() {
      ...
      menu
        .findChild(".export-layers")
        .css("display", checkboxes.length < 2 ? "none" : "block");
    }
  ...
  function LayerControl(gui) {
    ...
    function getLayerOrderSlug() {
      return internal
        .sortLayersForMenuDisplay(model.getLayers())
        .map(function(o) {
    ...
    function initLayerDragging(entry, id) {
      ...
      entry.on("mousemove", function(e) {
        ...
        insertionClass =
          e.pageY - rect.top < rect.height / 2
            ? "insert-above"
            : "insert-below";
